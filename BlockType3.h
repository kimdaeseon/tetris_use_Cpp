...
void BlockType3::print() {      //현재 위치에 맞게 커서를 이동한 후 블럭 출력
...
void BlockType3::del() {          // 현재 위치에 맞게 커서를 이동한후 ' '출력
...
void BlockType3::operator+(char board[][boardSize]) {   // 현재 블록의 위치에 맞게 테트리스 보드 원소를 127로 초기화
	for (int i = 0; i < Blocksize.x; i++) {
		for (int j = 0; j < Blocksize.y; j++) {
			if (form[i][j] == 127)
...
void BlockType3::change() {                              // 회전할때의 모양의 맞게 블록의 크기 변수를 수정하고
